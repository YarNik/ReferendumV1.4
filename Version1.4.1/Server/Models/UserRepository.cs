...
namespace WebServer.Models
{
    ...
    public class UserRepository : IUserRepository
    {
        ...
        public List<GetReferendums> GetReferendumsDbNew(User user)
        {
            ...
            {
                return db.Query<GetReferendums>("SELECT Referendum.Id, Referendum.Proposition, Answer.Appellation, Answer.id AS IdAnswer, COUNT(*) - 1  AS Amount, Referendum.MaxAmountAnswers - (SELECT COUNT(*) FROM AllAnswers WHERE AllAnswers.IdUser = @Id AND AllAnswers.idAnswer = Answer.id) AS VotesAmount, Referendum.DeadLine FROM Referendum, Answer, AllAnswers WHERE Answer.Referendum = Referendum.id AND AllAnswers.idAnswer = Answer.id GROUP BY Answer.id, Answer.Appellation, Referendum.Proposition, Referendum.MaxAmountAnswers, Referendum.DeadLine, Referendum.Id", new { Id = user.Id }).ToList();            
            }
        }
